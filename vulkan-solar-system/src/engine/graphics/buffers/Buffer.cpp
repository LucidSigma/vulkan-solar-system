...
void Buffer::Create(const void* bufferData, const VkDeviceSize bufferSize, const Usage usage)
{
	...
	vmaMapMemory(m_renderer.GetVulkanContext().GetAllocator(), stagingAllocation, &data);
	{
		std::memcpy(data, bufferData, static_cast<std::size_t>(bufferSize));
	}
	vmaUnmapMemory(m_renderer.GetVulkanContext().GetAllocator(), stagingAllocation);

	vulkan_util::BufferInfo bufferInfo{ };
	bufferInfo.size = bufferSize;
	bufferInfo.usageFlags = VK_BUFFER_USAGE_TRANSFER_DST_BIT | static_cast<VkBufferUsageFlagBits>(usage);
