...
Texture::Texture(const Renderer& renderer, const std::string_view& filepath, const bool generateMipmaps)
	: m_renderer(renderer)
{
	Initialise(filepath, generateMipmaps);
}
...
void Texture::Initialise(const std::string_view& filepath, const bool generateMipmaps)
{
	int textureWidth = 0;
	int textureHeight = 0;

	stbi_uc* pixels = stbi_load(filepath.data(), &textureWidth, &textureHeight, nullptr, STBI_rgb_alpha);

	if (pixels == nullptr)
	{
		throw std::invalid_argument("Failed to load texture image: " + std::string(filepath));
	}
	...
	vulkan_util::CopyBufferToImage(m_renderer.GetVulkanContext(), stagingBuffer, m_image, static_cast<std::uint32_t>(textureWidth), static_cast<std::uint32_t>(textureHeight));
	...
	m_imageView = vulkan_util::CreateImageView(m_renderer.GetVulkanContext(), m_image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, m_mipLevels);
}
