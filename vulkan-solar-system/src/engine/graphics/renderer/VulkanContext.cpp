...
void VulkanContext::SelectPhysicalDevice()
{
	...
	if (std::cbegin(physicalDeviceScores)->first > 0)
	{
		...
		m_maxAnisotropy = physicalDeviceProperties.limits.maxSamplerAnisotropy;
	}
	else
	{
		throw std::runtime_error("Failed to find a suitable GPU that supports Vulkan.");
	}
}
...
[[nodiscard]] std::uint64_t VulkanContext::GetPhysicalDeviceScore(const VkPhysicalDevice physicalDevice) const
{
	...
	score += static_cast<std::uint64_t>(physicalDeviceProperties.limits.maxSamplerAnisotropy * 100.0f);
...
[[nodiscard]] VkSampleCountFlagBits VulkanContext::GetMaxUsableSampleCount() const
{
	VkPhysicalDeviceProperties physicalDeviceProperties{ };
	vkGetPhysicalDeviceProperties(m_physicalDevice, &physicalDeviceProperties);

	VkSampleCountFlags sampleCounts = physicalDeviceProperties.limits.framebufferColorSampleCounts & physicalDeviceProperties.limits.framebufferDepthSampleCounts & physicalDeviceProperties.limits.framebufferStencilSampleCounts;
	
	constexpr std::array<VkSampleCountFlagBits, 6u> AllSampleCounts{
		VK_SAMPLE_COUNT_64_BIT, VK_SAMPLE_COUNT_32_BIT, VK_SAMPLE_COUNT_16_BIT,
		VK_SAMPLE_COUNT_8_BIT, VK_SAMPLE_COUNT_4_BIT, VK_SAMPLE_COUNT_2_BIT
	};

	for (const auto sampleCount : AllSampleCounts)
	{
		if (sampleCounts & sampleCount)
		{
			return sampleCount;
		}
	}
