...
Renderer::Renderer(const Window& window, const Config& config)
	: m_window(window), m_vulkanContext(m_window), m_enableVsync(config.enableVsync), m_enableMultisample(config.enableMultisample), m_depthStencilBuffer(m_vulkanContext), m_msaaBuffer(m_vulkanContext)
{
	InitialisePipelineCache();
	InitialiseSynchronisationPrimitives();
	
	InitialisePresentationObjects();

	InitialiseShaderCompiler();
}
...
bool Renderer::PrepareRender()
{
	...
	if (m_currentImagesInFlight[m_currentFrameInFlight] != VK_NULL_HANDLE)
	{
		vkWaitForFences(m_vulkanContext.GetLogicalDevice(), 1, &m_currentImagesInFlight[m_currentFrameInFlight], VK_TRUE, std::numeric_limits<std::uint64_t>::max());
	}

	m_currentImagesInFlight[m_currentFrameInFlight] = m_perFrameSynchronisation[m_currentFrameInFlight].inFlightFence;
...
void Renderer::DrawInstanced(const std::uint32_t vertexCount, const std::uint32_t instanceCount)
{
	vkCmdDraw(m_commandBuffers[m_nextAcquiredImageIndex], vertexCount, instanceCount, 0, 0);
}
...
void Renderer::DrawInstancedIndex(const std::uint32_t indexCount, const std::uint32_t instanceCount)
{
	vkCmdDrawIndexed(m_commandBuffers[m_nextAcquiredImageIndex], indexCount, instanceCount, 0, 0, 0);
}
...
void Renderer::InitialisePresentationObjects()
{
	InitialiseSwapchain();
	InitialiseRenderPass();
	m_msaaBuffer.Initialise(m_swapchainExtent, m_surfaceFormat.format);
	m_depthStencilBuffer.Initialise(m_swapchainExtent, m_enableMultisample);
...
void Renderer::InitialiseSwapchain()
{
	...
	m_currentImagesInFlight.clear();
	m_currentImagesInFlight.resize(m_swapchainImages.size(), VK_NULL_HANDLE);
}
...
void Renderer::InitialiseSwapchainImages()
{
	...
	{
		m_swapchainImageViews[i] = vulkan_util::CreateImageView(m_vulkanContext, m_swapchainImages[i], m_surfaceFormat.format, VK_IMAGE_ASPECT_COLOR_BIT, 1);
	}
}
...
[[nodiscard]] VkPresentModeKHR Renderer::GetBestPresentationMode(const std::vector<VkPresentModeKHR>& availablePresentationModes) const
{
	...
	{
		if (availablePresentationMode == VK_PRESENT_MODE_MAILBOX_KHR && m_enableVsync)
		{
	...
	if (!m_enableVsync && !supportsImmediateMode)
	{
		throw std::runtime_error("Failed to get non-Vsync Vulkan presenation format.");
	}
...
void Renderer::InitialiseRenderPass()
{
	std::vector<VkAttachmentDescription> attachmentDescriptions;
	{
		...
		{
			// MSAA attachment.
			attachmentDescriptions.back().format = m_surfaceFormat.format;
			attachmentDescriptions.back().samples = m_vulkanContext.GetMSAASampleCount();
			attachmentDescriptions.back().loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
			attachmentDescriptions.back().storeOp = VK_ATTACHMENT_STORE_OP_STORE;
			attachmentDescriptions.back().stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
			attachmentDescriptions.back().stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
			attachmentDescriptions.back().initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
			attachmentDescriptions.back().finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
		}
		else
		{
			// Non-sampled colour attachment.
			attachmentDescriptions.back().format = m_surfaceFormat.format;
			attachmentDescriptions.back().samples = VK_SAMPLE_COUNT_1_BIT;
			attachmentDescriptions.back().loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
			attachmentDescriptions.back().storeOp = VK_ATTACHMENT_STORE_OP_STORE;
			attachmentDescriptions.back().stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
			attachmentDescriptions.back().stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
		...
		attachmentDescriptions.emplace_back();
		attachmentDescriptions.back().format = m_depthStencilBuffer.GetFormat();
		attachmentDescriptions.back().samples = m_enableMultisample ? m_vulkanContext.GetMSAASampleCount() : VK_SAMPLE_COUNT_1_BIT;
		attachmentDescriptions.back().loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
		attachmentDescriptions.back().storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
		attachmentDescriptions.back().stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
		attachmentDescriptions.back().stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
		attachmentDescriptions.back().initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
		attachmentDescriptions.back().finalLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;

		if (m_enableMultisample)
		{
			// Single-sample colour attachment.
			attachmentDescriptions.emplace_back();
			attachmentDescriptions.back().format = m_surfaceFormat.format;
			attachmentDescriptions.back().samples = VK_SAMPLE_COUNT_1_BIT;
			attachmentDescriptions.back().loadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
			attachmentDescriptions.back().storeOp = VK_ATTACHMENT_STORE_OP_STORE;
			attachmentDescriptions.back().stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
			attachmentDescriptions.back().stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
			attachmentDescriptions.back().initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
	...
	std::array<VkAttachmentReference, 1u> subpassMultisampledAttachmentReferences{ };
	{
		subpassMultisampledAttachmentReferences[0].attachment = 0;
		subpassMultisampledAttachmentReferences[0].layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
	}
	...
	{
		subpassColourAttachmentReferences[0].attachment = 2;
	...
	std::array<VkSubpassDescription, 1u> subpassDescriptions{ };
	{
		...
		subpassDescriptions[0].colorAttachmentCount = static_cast<std::uint32_t>(subpassMultisampledAttachmentReferences.size());
		subpassDescriptions[0].pColorAttachments = subpassMultisampledAttachmentReferences.data();
		subpassDescriptions[0].pResolveAttachments = m_enableMultisample ? subpassColourAttachmentReferences.data() : nullptr;
	}
...
void Renderer::InitialiseFramebuffers()
{
	...
	for (std::size_t i = 0; i < m_swapchainImageViews.size(); ++i)
	{
		...
		framebufferAttachments.push_back(m_enableMultisample ? m_msaaBuffer.GetImageView() : m_swapchainImageViews[i]);
		framebufferAttachments.push_back(m_depthStencilBuffer.GetImageView());
		
		if (m_enableMultisample)
		{
			framebufferAttachments.push_back(m_swapchainImageViews[i]);
		}
...
void Renderer::InitialiseShaderCompiler()
{
	m_shaderCompiler = shaderc_compiler_initialize();

	if (m_shaderCompiler == nullptr)
	{
		throw std::runtime_error("Failed to initialise ShaderC shader compiler.");
	}
}

void Renderer::DestroyShaderCompiler() noexcept
{
	if (m_shaderCompiler != nullptr)
	{
