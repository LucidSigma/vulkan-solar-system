...
void GraphicsPipeline::SetTexture(const std::uint32_t binding, const Texture& texture, const Sampler& sampler) const
{
	if (m_boundTextures.find(binding) != std::cend(m_boundTextures) && m_boundTextures[binding].first == &texture && m_boundTextures[binding].second == &sampler)
	{
		return;
	}

	UpdateSamplerDescriptor(binding, texture.GetImageView(), sampler.GetHandle());
	m_boundTextures[binding] = { &texture, &sampler };
}

void GraphicsPipeline::SetTexture(const std::uint32_t binding, const Cubemap& cubemap, const Sampler& sampler) const
{
	if (m_boundCubemaps.find(binding) != std::cend(m_boundCubemaps) && m_boundCubemaps[binding].first == &cubemap && m_boundCubemaps[binding].second == &sampler)
	{
		return;
	}

	UpdateSamplerDescriptor(binding, cubemap.GetImageView(), sampler.GetHandle());
	m_boundCubemaps[binding] = { &cubemap, &sampler };
}
	...
	for (const auto& shaderModule : shaderModules)
	{
		if (shaderModule->GetStage() == ShaderModule::Stage::Vertex)
		{
			...
			for (const auto& shaderInput : shaderResources.stage_inputs)
			{
				...
				if (currentVertexInput.format == VK_FORMAT_UNDEFINED || currentVertexInput.size == 0)
				{
...
void GraphicsPipeline::InitialisePipelineLayout(const Config& config, const std::vector<std::unique_ptr<ShaderModule>>& shaderModules)
{
	...
	pipelineLayoutCreateInfo.pushConstantRangeCount = m_hasPushConstants ? 1 : 0;
	pipelineLayoutCreateInfo.pPushConstantRanges = m_hasPushConstants ? &m_pushConstantRange : nullptr;
...
void GraphicsPipeline::InitialisePipeline(const Config& config)
{
	...
	{
		shaderModules.emplace_back(std::make_unique<ShaderModule>(m_renderer.GetVulkanContext().GetLogicalDevice(), m_renderer.GetShaderCompiler(), shaderFilepath, shaderType, config.areShadersPreCompiled));
	...
	rasterisationStateCreateInfo.cullMode = config.cullFace != Config::CullFace::None ? VK_CULL_MODE_BACK_BIT : VK_CULL_MODE_NONE;
	rasterisationStateCreateInfo.frontFace = config.cullFace == Config::CullFace::Clockwise ? VK_FRONT_FACE_CLOCKWISE : VK_FRONT_FACE_COUNTER_CLOCKWISE;
	...
	multisampleStateCreateInfo.rasterizationSamples = m_renderer.IsMultisamplingEnabled() ? m_renderer.GetVulkanContext().GetMSAASampleCount() : VK_SAMPLE_COUNT_1_BIT;
	multisampleStateCreateInfo.sampleShadingEnable = m_renderer.IsMultisamplingEnabled() ? VK_TRUE : VK_FALSE;
	multisampleStateCreateInfo.minSampleShading = m_renderer.IsMultisamplingEnabled() ? 0.2f : 0.0f;
	...
	case Config::Blending::Additive:
		colourBlendAttachmentState.blendEnable = VK_TRUE;
		colourBlendAttachmentState.colorWriteMask = VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT;
		colourBlendAttachmentState.colorBlendOp = VK_BLEND_OP_ADD;
		colourBlendAttachmentState.srcColorBlendFactor = VK_BLEND_FACTOR_ONE;
		colourBlendAttachmentState.dstColorBlendFactor = VK_BLEND_FACTOR_ONE;
		colourBlendAttachmentState.alphaBlendOp = VK_BLEND_OP_ADD;
		colourBlendAttachmentState.srcAlphaBlendFactor = VK_BLEND_FACTOR_SRC_ALPHA;
		colourBlendAttachmentState.dstAlphaBlendFactor = VK_BLEND_FACTOR_DST_ALPHA;

		break;

	case Config::Blending::None:
	default:
	...
	if (vkCreateGraphicsPipelines(m_renderer.GetVulkanContext().GetLogicalDevice(), m_renderer.GetPipelineCache(), 1, &graphicsPipelineCreateInfo, nullptr, &m_pipelineHandle) != VK_SUCCESS)
	{
...
void GraphicsPipeline::InitialiseDescriptorSets()
{
	...
	for (std::size_t i = 0; i < m_renderer.GetSwapchainImageCount(); ++i)
	{
		for (const auto& [binding, bindingData] : m_bindingsData)
		{
			...
			{
				if (m_boundTextures.find(binding) == std::cend(m_boundTextures) && m_boundCubemaps.find(binding) == std::cend(m_boundCubemaps))
				{
					continue;
				}
				...
				if (m_boundTextures.find(binding) != std::cend(m_boundTextures))
				{
					descriptorImageInfo.imageView = m_boundTextures[binding].first->GetImageView();
					descriptorImageInfo.sampler = m_boundTextures[binding].second->GetHandle();
				}
				else
				{
					descriptorImageInfo.imageView = m_boundCubemaps[binding].first->GetImageView();
					descriptorImageInfo.sampler = m_boundCubemaps[binding].second->GetHandle();
				}
			}
...
void GraphicsPipeline::SetUniformBufferData(const std::uint32_t updatedBinding, const void* data)
{
	...
	for (const auto& [binding, bindingData] : m_bindingsData)
	{
		...
		else if (bindingData.type == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER)
		{
...
void GraphicsPipeline::UpdateSamplerDescriptor(const std::uint32_t updatedBinding, const VkImageView imageView, const VkSampler samplerHandle) const
{
	for (std::size_t i = 0; i < m_renderer.GetSwapchainImageCount(); ++i)
	{
		VkDescriptorImageInfo descriptorImageInfo{ };
		...
		descriptorImageInfo.sampler = samplerHandle;

		VkWriteDescriptorSet writeDescriptorSet{ };
		writeDescriptorSet.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		writeDescriptorSet.dstSet = m_descriptorSets[i];
		writeDescriptorSet.dstBinding = updatedBinding;
		writeDescriptorSet.dstArrayElement = 0;
