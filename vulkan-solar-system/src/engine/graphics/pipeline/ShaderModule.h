...
public:
	ShaderModule(const VkDevice vulkanDevice, const shaderc_compiler_t compiler, const std::string& shaderFilepath, const Stage stage, const bool preCompiled = false);
...
private:
	...
	[[nodiscard]] std::vector<std::uint32_t> ReadGLSLFile(const std::string& filepath);
